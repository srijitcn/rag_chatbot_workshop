...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=data-science&org_id=3393959132497595&notebook=%2F_resources%2F00-init&demo_name=llm-rag-chatbot&event=VIEW&path=%2F_dbdemos%2Fdata-science%2Fllm-rag-chatbot%2F_resources%2F00-init&version=1">
...
# DBTITLE 1,Copy the raw documentation from master schema
raw_documentation_master_table = f"{master_db_name}.raw_documentation"
raw_documentation_table = f"{catalog}.{dbName}.raw_documentation"

if not spark.catalog.tableExists("raw_documentation"):
    (spark.sql(f"SELECT * FROM {raw_documentation_master_table}")
    .write
    .mode("overwrite")
    .saveAsTable(raw_documentation_table))
